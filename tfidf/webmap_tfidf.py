...
cursor.execute("TRUNCATE TABLE WebMap_tfidf")
cursor.execute("TRUNCATE TABLE WebMap_tfidf_value")

sql = "select title, context, description, tourist_sign, id from Webmap;"
cursor.execute(sql)
...
for fv,record in zip(fv_tf_idf,records):
    ...
    print
    j=0
    count = 0   #tf-idf値が5番目まで高いものをカウントするための変数
    top_tfidf = []
    tfidf_value = []
    ...
    for i in range(5):
        if not top_tfidf[i]:
            top_tfidf.append("")
        if not tfidf_value[i]:
            tfidf_value.append("")

    cursor.execute('INSERT INTO WebMap_tfidf (webmap_id, tfidf1, tfidf2, tfidf3, tfidf4, tfidf5)\
      VALUES( %s, %s, %s, %s, %s, %s)', (id, top_tfidf[0], top_tfidf[1], top_tfidf[2], top_tfidf[3], top_tfidf[4]))
    connector.commit()
    cursor.execute('INSERT INTO WebMap_tfidf_value (webmap_id, vTfidf1, vTfidf2, vTfidf3, vTfidf4, vTfidf5)\
      VALUES( %s, %s, %s, %s, %s, %s)', (id, tfidf_value[0], tfidf_value[1], tfidf_value[2], tfidf_value[3], tfidf_value[4]))
    connector.commit()
f.close()
connector.close()
